...
if WITH_CUDA:
    MAP_FN = lambda storage, loc: storage.cuda(DEVICE_ID)
elif WITH_XPU:
    MAP_FN = lambda storage, loc: storage.xpu(DEVICE_ID)
else:
    MAP_FN = lambda storage, loc: storage.cpu()
...
class Trainer:
    def __init__(self, 
        ...
        assert len(self.cfg.gnn_outputs_path) == len(self.cfg.traj_data_path) and \
            len(self.cfg.gnn_outputs_path) == len(self.cfg.size_list), \
            'gnn_outputs_path, traj_data_path, and size_list must have the same number of elements'
        assert sum(self.cfg.size_list) == SIZE, \
            'sum of size_list must equal total number of ranks'
        assert self.cfg.time_dependency == 'time_dependent', \
            'Current version only supports time_dependent.'
        assert self.cfg.online == False, \
            'Current version only supports offline training.'
        # if self.cfg.model_task == 'inference':
        #     assert len(self.cfg.size_list) == 1, \
        #         'Current version only supports one case for inference'
        ...
        if WITH_DDP:
            ...
            # MASTER_ADDR="x4220c3s0b0n0"
            MASTER_ADDR = str(cfg.master_addr)
            # MASTER_ADDR = MPI.COMM_WORLD.bcast(MASTER_ADDR, root=0)
            if MASTER_ADDR is None:
                log.info(f"[RANK {RANK}] -- MASTER_ADDR is None")
        ...
        # ~~~~ Map ranks to cases
        if SIZE > 1:
            # find first j where RANK < cum[j]
            cum = []
            total = 0
            for sz in self.cfg.size_list:
                total += sz
                cum.append(total)
            for j, end in enumerate(cum):
                if RANK < end:
                    case_id = j
                    break
            start = cum[case_id-1] if case_id > 0 else 0
            local_rank = RANK - start
            self.CASE_ID          = case_id
            self.CASE_RANK_START  = start
            self.CASE_RANK        = local_rank
            self.CASE_SIZE        = self.cfg.size_list[case_id]
            self.gnn_outputs_path = self.cfg.gnn_outputs_path[case_id]
            self.traj_data_path   = self.cfg.traj_data_path[case_id]
            self.size_list        = self.cfg.size_list[case_id]
            self.num_cases        = len(self.cfg.size_list)
        else:
            self.gnn_outputs_path = self.cfg.gnn_outputs_path[0]
            self.traj_data_path = self.cfg.traj_data_path[0]
            self.size_list = self.cfg.size_list[0]
            self.CASE_RANK = RANK
            self.CASE_SIZE = self.cfg.size_list[0]
            self.CASE_RANK_START = 0
            self.CASE_ID = 0
            self.num_cases = 1
        if self.cfg.verbose: 
            log.info(f"[RANK {RANK}] -- CASE_ID: {self.CASE_ID}, CASE_RANK: {self.CASE_RANK}, CASE_SIZE: {self.CASE_SIZE}, CASE_RANK_START: {self.CASE_RANK_START}")

        # ~~~~ Build case groups - used to conditionally broadcast data for ranks belonging to the same case
        cum_sizes = list(itertools.accumulate(self.cfg.size_list))
        case_ranks = [] # list of size n_cases, each element has the rank ids for that case 
        start = 0
        for sz in self.cfg.size_list:
            case_ranks.append(list(range(start, start+sz))) 
            start += sz 
        self.case_groups = [
            dist.new_group(ranks=ranks, backend=dist.get_backend()) 
            for ranks in case_ranks
        ]
        self.case_group = self.case_groups[self.CASE_ID]
        self.case_start_id = []
        for i in range(self.num_cases):
            self.case_start_id.append(case_ranks[i][0])

        if self.cfg.verbose:
            log.info(f"[RANK {RANK}] -- backend: {dist.get_backend()}, case_ranks: {case_ranks}, case_group: {self.case_group}")
        ...
        self.setup_halo()
        if RANK == 0: log.info('Done with setup_halo')
        ...
        # ~~~~ Setup halo exchange masks -- now do this 
        ...
        if self.cfg.restart:
            ...
            # ckpt = torch.load(self.ckpt_path, map_location=MAP_FN)
        ...
        if self.cfg.model_task == 'inference':
            ...
            # ckpt = torch.load(self.model_path, map_location=MAP_FN)
    ...
    def build_model(self) -> nn.Module:
        ...
        # Full model
        input_node_channels = sample['x'].shape[1] + 1 + 1 # +1 for Reynolds number, +1 for dt
        input_edge_channels = graph.edge_attr.shape[1]
        hidden_channels = self.cfg.hidden_channels
        output_node_channels = sample['y'][0].shape[1]
        ...
        if self.cfg.model_task == 'inference':
            name = 'POLY_3_SIZE_12_SEED_12' # hardcode 
    ...
    def load_graph_data(self):
        ...
        if not self.cfg.online:
            main_path = self.gnn_outputs_path + '/'
        else:
            main_path = ""
        ...
        if self.cfg.client.backend == 'adios':
            ...
            path_to_pos_full = main_path + 'pos_node_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
            path_to_ei = main_path + 'edge_index_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
            path_to_overlap = main_path + 'overlap_ids_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
            path_to_glob_ids = main_path + 'global_ids_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
            path_to_unique_local = main_path + 'local_unique_mask_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
            path_to_unique_halo = main_path + 'halo_unique_mask_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
            path_to_Np = main_path + "Np_rank_%d_size_%d" %(self.CASE_RANK, self.CASE_SIZE)
    ...
    def setup_halo(self):
        if SIZE > 1 and self.cfg.consistency:
            ...
            if not self.cfg.online:
                path_to_ew = self.gnn_outputs_path + '/edge_weights_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
                path_to_node_degree = self.gnn_outputs_path + '/node_degree_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
                path_to_halo_info = self.gnn_outputs_path + '/halo_info_rank_%d_size_%d' %(self.CASE_RANK,self.CASE_SIZE)
                ...
                halo_info[:,3] = halo_info[:,3] + self.CASE_RANK_START # increment by case rank
            else:
    ...
    def load_field_data(self, data_dir: str):
        ...
        if 'stats' not in self.data.keys():
            if os.path.exists(data_dir + f"/data_stats.npz") and self.cfg.load_stats:
                #if RANK == 0:
    ...
    def load_trajectory(self, data_dir: str):
        ...
        if not self.cfg.online:
            # COMM.Barrier()
            log.info(f"Loading trajectory data from {data_dir}")
            # log.info(f"[RANK {RANK}] - case rank: {self.CASE_RANK}, case size: {self.CASE_SIZE}")
            # utils.force_abort()
            files = os.listdir(data_dir+f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}")
            #files = [item for item in files_temp if 'p_step' not in item]
            files.sort(key=lambda x:int(x.split('_')[-1].split('.')[0]))

            t_files = [f for f in files if f.startswith("t_step_")]
            files = [f for f in files if f.startswith("u_step_")]
        
            # Get the Reynolds number here - this is supplied in input file. 
            # Compute tau_k = 1/sqrt(Re) as the normalization time scale
            if self.cfg.model_task == "train":
                reynolds_number = torch.tensor(self.cfg.reynolds_number_list[self.CASE_ID])
            elif self.cfg.model_task == "inference":
                reynolds_number = torch.tensor(self.cfg.reynolds_number_inference)
            re_max = torch.tensor(self.cfg.reynolds_number_list).max()
            re_min = torch.tensor(self.cfg.reynolds_number_list).min()
            tau_k = torch.sqrt(1./reynolds_number) # approximation of Kolmogorov time scale

            max_rollout = self.cfg.max_rollout_steps
            n_files = len(files)
            idx = list(range(n_files))

            for i in range(n_files - max_rollout):
                # ~~~~ Load x snapshot
                step_x_i = idx[i] 
                path_x_i = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + files[step_x_i]
                data_x_i = self.load_data(path_x_i, dtype=np.float64).reshape((-1,3))
                data_x_i = self.prepare_snapshot_data(data_x_i)
                tpath_x_i = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + t_files[step_x_i]
                time_x_i = torch.tensor(np.loadtxt(tpath_x_i))

                # ~~~~ Collect y snapshots and times 
                y_list_i = []
                time_y_list_i = []
                step_y_list_i = []
                for m in range(1, max_rollout+1):
                    step_y_i_m = idx[i + m]
                    path_y_i_m = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + files[step_y_i_m]
                    data_y_i_m = self.load_data(path_y_i_m, dtype=np.float64).reshape((-1,3))
                    data_y_i_m = self.prepare_snapshot_data(data_y_i_m)
                    tpath_y_i_m = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + t_files[step_y_i_m]
                    time_y_i_m = torch.tensor(np.loadtxt(tpath_y_i_m))
                    y_list_i.append(data_y_i_m)
                    time_y_list_i.append(time_y_i_m)
                    step_y_list_i.append(step_y_i_m)

                # ~~~~ Collect dt
                dt_list_i = []
                for j, t_y in enumerate(time_y_list_i):
                    if j == 0:
                        dt_list_i.append(t_y - time_x_i)
                    else:
                        dt_list_i.append(t_y - time_y_list_i[j-1])
                
                # ~~~~ Create dict for data_list 
                self.data_list.append({
                    'x': data_x_i,
                    'y': y_list_i, 
                    'step_x': step_x_i,
                    'step_y': step_y_list_i,
                    're': reynolds_number,
                    'time_x': time_x_i,
                    'time_y': time_y_list_i,
                    'dt': dt_list_i,
                })
            ...
            # idx_x = idx[:-1]
            # idx_y = idx[1:]
            # if RANK == self.CASE_RANK_START: log.info(f"[CASE {self.CASE_ID}] -- Loading {len(files)} trajectory data files ...")
            # for i in range(len(idx_x)):
            #     step_x_i = idx_x[i]
            #     step_y_i = idx_y[i]
            #     path_x_i = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + files[idx_x[i]]
            #     path_y_i = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + files[idx_y[i]]
            #     data_x_i = self.load_data(path_x_i, dtype=np.float64).reshape((-1,3))
            #     data_y_i = self.load_data(path_y_i, dtype=np.float64).reshape((-1,3))
            #     data_x_i = self.prepare_snapshot_data(data_x_i)
            #     data_y_i = self.prepare_snapshot_data(data_y_i)
            #     path_xtime_i = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + t_files[idx_x[i]]
            #     path_ytime_i = data_dir + f"/data_rank_{self.CASE_RANK}_size_{self.CASE_SIZE}/" + t_files[idx_y[i]]
            #     time_x_i = torch.tensor(np.loadtxt(path_xtime_i)) 
            #     time_y_i = torch.tensor(np.loadtxt(path_ytime_i))
            #     dt_i = time_y_i - time_x_i
            #     self.data_list.append(
            #             {'x': data_x_i, 'y':data_y_i, 'step_x':step_x_i, 'step_y':step_y_i, 're': reynolds_number, 'time_x':time_x_i, 'time_y':time_y_i, 'dt': dt_i} 
            #     )
        ...
        if RANK == self.CASE_RANK_START: log.info(f"[CASE {self.CASE_ID}] -- Number of training snapshots: {len(data['train'])}")
        if RANK == self.CASE_RANK_START: log.info(f"[CASE {self.CASE_ID}] -- Number of validation snapshots: {0}")

        # Compute statistics for normalization
        stats = {'x': [], 'y': []} 
        if 'stats' not in self.data.keys():
            if os.path.exists(data_dir + "/data_stats.npz") or self.cfg.load_stats:
                #if RANK == 0:
                npzfile = np.load(data_dir + "/data_stats.npz")
                stats['x'] = [npzfile['x_mean'], npzfile['x_std']]
                stats['y'] = [npzfile['y_mean'], npzfile['y_std']]
                #stats = COMM.bcast(stats, root=0)
                if RANK == self.CASE_RANK_START: log.info(f"[CASE {self.CASE_ID}] -- Read training data statistics from {data_dir}/data_stats.npz")
            else: 
                if RANK == 0: log.info(f"Computing training data statistics")
                x_mean, x_std = self.compute_statistics(data['train'],'x')
                if RANK == self.CASE_RANK_START and not self.cfg.online:
                ...
                if RANK == 0: log.info(f"Computed training data statistics for each node feature")
        # Add Reynolds stats and Kolmogorov time scale  
        stats['re'] = [re_min, re_max - re_min]
        stats['tau_k'] = tau_k
        if RANK == 0: log.info(f"stats: {stats}")
        return data, stats
    ...
    def setup_data(self):
        ...
        # Normalize edge_attrs by length of the longest edge - we do the reduction over all cases
        # TODO: Q: do we need to do all_reduce over all cases, or localize to case_group?
        ...
        if (RANK == self.CASE_RANK_START):
            log.info(f"[CASE {self.CASE_ID}] -- {data_graph}")
            log.info(f"[CASE {self.CASE_ID}] -- shape of x: {data['train'][0]['x'].shape}")
            log.info(f"[CASE {self.CASE_ID}] -- shape of y: {data['train'][0]['y'][0].shape}")
        ...
        for item in data['train']:
            ...
            tdict['y'] = [None] * self.cfg.max_rollout_steps
            tdict['dt'] = [None] * self.cfg.max_rollout_steps
            for m in range(self.cfg.max_rollout_steps):
                tdict['y'][m] = ((item['y'][m] - stats['y'][0])/(stats['y'][1] + SMALL)).to(self.torch_dtype)
                tdict['dt'][m] = (item['dt'][m]/stats['tau_k']).to(self.torch_dtype)
            tdict['re'] = ((item['re'] - stats['re'][0])/(stats['re'][1] + SMALL)).to(self.torch_dtype)
            train_data_scaled.append(tdict)
        train_loader = DataLoader(dataset=train_data_scaled,
                                     batch_size=self.cfg.batch_size,
                                     shuffle=True)

        # Set the last training snapshot as the validation snapshot 
        val_data_scaled = [train_data_scaled[-1]]
        ...
        if self.cfg.model_task == 'inference':
            inference_data_scaled = train_data_scaled
            inference_loader = DataLoader(dataset=inference_data_scaled,
                                            batch_size=self.cfg.batch_size,
                                            shuffle=False)
        ...
        self.data =  {
            ...
            'inference': {
                'loader': inference_loader if self.cfg.model_task == 'inference' else None,
            },
            'graph': data_graph
        }
    ...
    def train_step(self, data) -> tuple[Tensor, list[Tensor]]:
        self.model.train()
        loss = torch.tensor([0.0])
        graph = self.data['graph']
        data['re'] = data['re'][:, None, None].expand(-1, data['x'].size(1), 1) # [1] -> [1,1,1], then expand to [1,N,1]
        tic = time.time()
        if WITH_CUDA or WITH_XPU:
            data['x'] = data['x'].to(self.device)
            for m in range(self.rollout_steps):
                data['y'][m] = data['y'][m].to(self.device)
                data['dt'][m] = data['dt'][m].to(self.device)
            data['re'] = data['re'].to(self.device)
        ...
        # Prediction
        x_scaled = data['x'][0]
        reynolds_number = data['re'][0]
        x_new = x_scaled
        for t in range(self.rollout_steps):
            x_old = torch.clone(x_new)
            tic = time.time()
            dt = data['dt'][t][:, None].expand(x_old.size(0), 1) # [1] -> [1,1], then expand to [N,1]
            x_old = torch.cat([x_old, reynolds_number, dt], dim=1)
            if self.cfg.pushforward_trick:
                context = torch.no_grad() if (self.rollout_steps > 1 and t < self.rollout_steps - 1) else contextlib.nullcontext()
            else:
            ...
            with context:
                if self.cfg.gradient_checkpointing:
                    ...
                    SIZE,
                    graph.batch)
                    out_gnn = tckpt.checkpoint(self.model, *args, use_reentrant=False)
                else:
                    out_gnn = self.model(x = x_old,
                                        ...
                                        SIZE = SIZE,
                                        batch = graph.batch)
                x_new = out_gnn + x_old[:,:-2] if self.cfg.use_residual else out_gnn
            ...
            if SIZE == 1 or not self.cfg.consistency:
                loss_t = self.loss_fn(x_new[:n_nodes_local], target[:n_nodes_local])
                effective_nodes = n_nodes_local 
            else: # custom 
                n_output_features = x_new.shape[1]
                squared_errors_local = torch.pow(x_new[:n_nodes_local] - target[:n_nodes_local], 2)
                ...
                effective_nodes = distnn.all_reduce(effective_nodes_local, op=dist.ReduceOp.SUM, group=self.case_group)
                sum_squared_errors = distnn.all_reduce(sum_squared_errors_local, op=dist.ReduceOp.SUM, group=self.case_group)
                loss_t = (1.0/(effective_nodes*n_output_features)) * sum_squared_errors
            if self.cfg.pushforward_trick:
                # only keep final step loss
                if t == self.rollout_steps - 1:
                    loss = loss_t
            else:
                # accumulate average loss over all steps
                loss += loss_t/self.rollout_steps
        ...
        # # Collect case-averaged losses and total average loss 
        # with torch.no_grad():
        #     # gather all per‐rank scalars into a list 
        #     temp_tensor = loss.detach().clone()
        #     gathered = [torch.zeros_like(temp_tensor) for _ in range(SIZE)]
        #     dist.all_gather(gathered, temp_tensor)

        #     # pick off the case‐leaders
        #     per_case_avgs = [gathered[i].item() for i in self.case_start_id]
        #     loss_total_avg = torch.tensor(sum(per_case_avgs)/self.num_cases)
        # return loss_total_avg, per_case_avgs
        return loss, None
    
    def inference_step(self, data_x, graph, dt, reynolds_number) -> tuple[Tensor, list[Tensor]]:
        self.model.eval()
        reynolds_number = reynolds_number[:, None, None].expand(-1, data_x.size(1), 1) # [1] -> [1,1,1], then expand to [1,N,1]
        if WITH_CUDA or WITH_XPU:
            data_x = data_x.to(self.device)
            reynolds_number = reynolds_number.to(self.device)
            dt[0] = dt[0].to(self.device)
            graph.edge_index = graph.edge_index.to(self.device)
            graph.edge_attr = graph.edge_attr.to(self.device)
            graph.batch = graph.batch.to(self.device) if graph.batch is not None else None
            graph.halo_info = graph.halo_info.to(self.device)
        ...
        # Prediction
        # TODO: make sure things are scaled properly.
        x_scaled = data_x[0]
        reynolds_number = reynolds_number[0]
        dt = dt[0][:, None].expand(x_scaled.size(0), 1) # [1] -> [1,1], then expand to [N,1]
        x_scaled = torch.cat([x_scaled, reynolds_number, dt], dim=1)
        ...
        out_gnn = self.model(x = x_scaled,
                            ...
                            SIZE = SIZE,
                            batch = graph.batch)
        x_new = out_gnn + x_scaled[:,:-2] if self.cfg.use_residual else out_gnn
    ...
    def test(self):
        ...
        self.model.eval()
        test_loader = self.data['validation']['loader']
        ...
        # return running_loss, per_case_avgs
    ...
    def update_rollout_steps(self, incr=1000):
        """ Update rollout steps used in training based on curriculum """
        threshold = self.cfg.phase1_steps + self.cfg.phase2_steps
        if self.iteration > threshold:  
            increments = (self.iteration - threshold) // incr + 1
            self.rollout_steps = min(1 + increments, self.cfg.max_rollout_steps) 
        return 
