...
def train(cfg: DictConfig,
    ...
    if cfg.restart:
        ...
        flat = torch.nn.utils.parameters_to_vector(trainer.model.parameters())
        if RANK == trainer.CASE_RANK_START:
            log.info(f"---[CASE {trainer.CASE_ID}] - RESTART - INITIAL VALIDATION LOSS]---")
            log.info(f"test_loss: {test_loss:.4e}")
            # log.info(f'test_loss_case_avgs={[f"{v:.4e}" for v in test_loss_case_avgs]}')
            log.info(f"param norm: {flat.norm():.6g}")
            log.info(f"param sum: {flat.sum():.6g}")
            log.info(f"--------------------------------")
    ...
    while True:
        ...
        for bidx, data in enumerate(train_loader):
            t_step = time.time()
            loss, _ = trainer.train_step(data)
            ...
            trainer.update_rollout_steps(incr=5)
            ...
            if RANK == 0:
                summary_train = ' '.join([
                    ...
                    f'rollout_steps={trainer.rollout_steps}',
                    # f'loss_case_avgs={[f"{v:.4e}" for v in loss_case_avgs]}'
                ])
                if cfg.postprocess:
                    summary_train += f' grad norm: {postproc_out[0]:.6g}'
            ...
            if RANK == trainer.CASE_RANK_START:
                log.info(
                    # f'[STEP {trainer.iteration}] '
                    f'[CASE {trainer.CASE_ID}] - loss={loss.item():.4e} '
                    f'r_loss={running_loss:.4e}'
                )
            COMM.Barrier()
            ...
            if trainer.iteration % cfg.ckptfreq == 0:
                ...
                if RANK == trainer.CASE_RANK_START: 
                    log.info(f"--------------------------------")
                    log.info(f"[CASE {trainer.CASE_ID}] - VALIDATION LOSS AT CHECKPOINT:")
                    log.info(f"test_loss: {test_loss:.4e}")
                    # log.info(f'test_loss_case_avgs={[f"{v:.4e}" for v in test_loss_case_avgs]}')
                    log.info(f"param norm: {flat.norm():.6g}")
                    log.info(f"param sum: {flat.sum():.6g}")
