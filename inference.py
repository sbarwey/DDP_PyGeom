...
def inference(cfg: DictConfig) -> None:
    ...
    """
    assert cfg.model_task == 'inference', "Inference function called with model_task != 'inference'"
    ...
    loader = trainer.data['inference']['loader']
    pos = graph.pos_orig 
    ...
    with torch.no_grad():
        for bidx, data in enumerate(loader):
            ...
            # Move stats and target to device 
            for key, val in stats.items():
                stats[key] = torch.tensor(val).to(trainer.device)
            data['y'][0] = data['y'][0].to(trainer.device)
            ...
            reynolds_number = data['re']
            dt = data['dt']
            pred_scaled = trainer.inference_step(x, graph, dt, reynolds_number)

            # unscale 
            pred = pred_scaled * stats['x_std'] + stats['x_mean']
            pred_ro = pred.clone().detach().unsqueeze(0)

            # get target 
            target_scaled = data['y'][0][0]
            target = target_scaled * stats['x_std'] + stats['x_mean']
